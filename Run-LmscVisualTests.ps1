...
$reportDirectory = Join-Path $artifactRoot ("Lmsc-Visual-" + $runTimestamp)
$generatedDrawingPath = Join-Path $reportDirectory "generated.dwg"
$scriptPath = Join-Path $reportDirectory "RunLmscVisualTests.scr"
...
Write-Host "Building RunCADVisualTests..."
...
$startInfo.EnvironmentVariables["MODELY_TEST_DWG"] = $generatedDrawingPath
$startInfo.EnvironmentVariables["MODELY_LMSC_ASSEMBLY_PATH"] = $lmscAssemblyPath
$startInfo.EnvironmentVariables["MODELY_VISUAL_REPORT_DIR"] = $reportDirectory

Write-Host "Running the LMSC visual test in AutoCAD Core Console..."
...
if ($output.Contains("MODELY_CAD_VISUAL_TEST_END|FAIL")) {
    throw "RunCADVisualTests reported a failure. Log: $logPath"
}

if (-not $output.Contains("MODELY_CAD_VISUAL_TEST_END|PASS")) {
    throw "RunCADVisualTests did not produce a completion marker. Log: $logPath"
}
...
foreach ($requiredArtifact in @(
    ...
    if (-not (Test-Path -LiteralPath $requiredArtifact -PathType Leaf)) {
        throw "Visual report artifact was not created: $requiredArtifact"
    }
}

Write-Host "PASS: LMSC generated facades A/B/C/D and produced the PDF/PNG visual report."
